...
void *clientHandler(Client *client)
{
    ...
    while(1)
    {
        ...
        else if(strcmp(msgBuffer, "/show") == 0) //Show all users in the chat room
        {
            strcpy(sendMsg, "\0");
        ...
        else if(strcmp(ptr, "/sendTo") == 0) //Send message to specified client
        {
            ...
            for(i=0; i<MAX_BUFFER; i++)
            {
                if(strcmp(ptr, room->userNames[i]) == 0)
                {
                    if(i != client->index)
                    {
                        ...
                        sprintf(sendMsg, "File From %s, accept? (/Y or /N)", client->userName);
                        write(room->clientSockets[i], sendMsg, strlen(sendMsg)+1);
                    }
                    else{
        ...
        else
        {
            ...
            printf("Pushing message to queue: \"%s\"\n", ptr);
